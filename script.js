...
if (quoteForm) {
  quoteForm.addEventListener('submit', (e) => {
    ...
    const rubro = document.getElementById('q-rubro').value.trim();
    const contacto = document.getElementById('q-contacto').value.trim();
    const mensaje = document.getElementById('q-mensaje').value.trim();

    const subject = encodeURIComponent(`Pedido de cotización - ${nombre}${apellido ? ' ' + apellido : ''}`);
